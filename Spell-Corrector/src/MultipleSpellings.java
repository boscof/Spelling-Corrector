...
public class MultipleSpellings {
	...
	public static void calculateTiered(String[] misspelt, String[] dict, String[] corrected, int misspelt_count,
		...
		try {
			System.setOut(new PrintStream(new FileOutputStream("output/multi/Tiered-output.txt")));
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
		...
		for (int i = 0; i < misspelt_count; i++) {
			count = 0;
			while (bestmatches[i][count] !=  null) {
				if (corrected[i].equals(bestmatches[i][count])) {
					right = right + 1.0;
					result = 'Y';
				} else {
					result = 'N';
				...
				sum = sum + 1.0;
				System.out.format("%-20s %-20s %-20s %-7c%n", misspelt[i], bestmatches[i][count], corrected[i], result);
				count++;
			}
		}
	...
	public static void calculateConsensus(String[] misspelt, String[] dict, String[] corrected, int misspelt_count,
		...
		try {
			System.setOut(new PrintStream(new FileOutputStream("output/multi/Consensus-output.txt")));
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
		...
		//Calculates the Damerau - Levenshtein and biGram distance between each mis-spelt word and dictionary entry
		for (int i = 0; i < misspelt_count; i++) {
		...
		for (int i = 0; i < misspelt_count; i++) {
			count = 0;
			min = editDistance.score(misspelt[i], matched[i][count]) + 
					anothereditDistance.score(misspelt[i], matched[i][count]);
			for (int j = position[i] + 1; j < size_of_dict; j++) {
				dist = editDistance.score(misspelt[i], dict[j]) + anothereditDistance.score(misspelt[i], dict[j]);
				if (min == dist) {
					count++;
